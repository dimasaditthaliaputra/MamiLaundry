...
namespace MamiKasir
{
    public partial class Raportjasacucian : Form
    {
        public Raportjasacucian()
        {
            InitializeComponent();
        }
        ...
        private void Raportjasacucian_Load(object sender, EventArgs e)
        {
            Koneksi.conn.Open();
            MySqlDataAdapter adapter = new MySqlDataAdapter("SELECT * FROM `tblpelanggan`", Koneksi.conn);
            DataSet ds = new DataSet();
            adapter.Fill(ds);
            DataTableLaporLayanan.DataSource = ds.Tables[0];
            Koneksi.conn.Close();
        }

        private void DataTableLaporLayanan_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
