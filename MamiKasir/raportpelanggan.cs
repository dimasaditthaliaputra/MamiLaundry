...
namespace MamiKasir
{
    public partial class raportpelanggan : Form
    {
        public raportpelanggan()
        {
            InitializeComponent();
        }
        ...
        private void raportpelanggan_Load(object sender, EventArgs e)
        {
            Koneksi.conn.Open();
            MySqlDataAdapter adapter = new MySqlDataAdapter("SELECT * FROM `tblpelanggan`", Koneksi.conn);
            DataSet ds = new DataSet();
            adapter.Fill(ds);
            DataTableLaporPelanggan.DataSource = ds.Tables[0];
            Koneksi.conn.Close();
        }
    }
}
