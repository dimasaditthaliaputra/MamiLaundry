...
namespace MamiKasir
{
    public partial class Raportpegawai : Form
    {
        public Raportpegawai()
        {
            InitializeComponent();
        }
        ...
        private void Raportpegawai_Load(object sender, EventArgs e)
        {
            Koneksi.conn.Open();
            MySqlDataAdapter adapter = new MySqlDataAdapter("SELECT * FROM `tblpelanggan`", Koneksi.conn);
            DataSet ds = new DataSet();
            adapter.Fill(ds);
            DataTableLaporPegawai.DataSource = ds.Tables[0];
            Koneksi.conn.Close();
        }
    }
}
