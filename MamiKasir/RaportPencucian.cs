...
namespace MamiKasir
{
    public partial class RaportPencucian : Form
    {
        public RaportPencucian()
        {
            InitializeComponent();
        }
        ...
        private void RaportPencucian_Load(object sender, EventArgs e)
        {
            Koneksi.conn.Open();
            MySqlDataAdapter adapter = new MySqlDataAdapter("SELECT * FROM `tblpelanggan`", Koneksi.conn);
            DataSet ds = new DataSet();
            adapter.Fill(ds);
            DataTableLaporOrder.DataSource = ds.Tables[0];
            Koneksi.conn.Close();
        }
    }
}
